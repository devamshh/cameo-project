...
const heroModeContent = {
  signage: {
    theme: "Signage",
    image: "images/gallery/commercial-building-sign-install-crane-wide.jpg"
  },
  wayfinding: {
    theme: "Wayfinding",
    image: "images/gallery/specialized-rehabilitation-hospital-pylon-onsite.jpg"
  },
  fabrication: {
    theme: "Fabrication",
    image: "images/gallery/cameo-workers-welding-on-site.png"
  }
};
...
function createExperienceRail() {
  const sections = Array.from(document.querySelectorAll("main [data-theme]")).filter((section) => {
    const rect = section.getBoundingClientRect();
    return section.offsetHeight > 120 || rect.height > 120;
  });
  ...
  sections.forEach((section, index) => {
    ...
    button.setAttribute("aria-label", `Go to ${section.dataset.theme || `section ${index + 1}`}`);
    button.innerHTML = `<span>${String(index + 1).padStart(2, "0")}</span><strong>${section.dataset.theme || "Section"}</strong>`;
  ...
  const observer = new IntersectionObserver(
    (entries) => {
      ...
      railButtons.forEach((button) => {
        button.classList.toggle("active", button.dataset.target === active.target.id);
      });
    },
    { threshold: [0.24, 0.42, 0.62], rootMargin: "-20% 0px -36% 0px" }
  );
...
function createActionDock() {
  ...
  dock.innerHTML = `
    <a href="mailto:hq@cameouae.com" aria-label="Email Cameo">Mail</a>
    <a href="tel:+97126766522" aria-label="Call Cameo">Call</a>
    <button type="button" aria-label="Back to top">Top</button>
  `;

  const topButton = dock.querySelector("button");
  if (topButton) {
...
function createCommandPalette() {
  const currentSections = Array.from(document.querySelectorAll("main [data-theme]")).map((section, index) => {
    if (!section.id) {
      section.id = `section-${index + 1}`;
    }

    return {
      label: section.dataset.theme || `Section ${index + 1}`,
      meta: "This page",
      action: () => section.scrollIntoView({ behavior: "smooth", block: "start" })
    };
  });

  const commands = [
    { label: "Home", meta: "Page", href: "index.html" },
    { label: "Work", meta: "Page", href: "work.html" },
    { label: "Services", meta: "Page", href: "services.html" },
    { label: "About", meta: "Page", href: "about.html" },
    { label: "Clients", meta: "Page", href: "clients.html" },
    { label: "Contact", meta: "Page", href: "contact.html" },
    { label: "Email Cameo", meta: "Action", href: "mailto:hq@cameouae.com" },
    ...currentSections
  ];

  const trigger = document.createElement("button");
  trigger.type = "button";
  trigger.className = "command-trigger magnetic";
  trigger.setAttribute("aria-expanded", "false");
  trigger.setAttribute("aria-controls", "command-palette");
  trigger.innerHTML = "<span>Explore</span>";
  ...
  palette.innerHTML = `
    ...
    <section class="command-palette__panel" role="dialog" aria-modal="true" aria-label="Quick navigation">
      <div class="command-palette__head">
        ...
        <button type="button" data-command-close aria-label="Close quick navigation">Close</button>
      </div>
      <input type="search" placeholder="Search pages, sections or actions" aria-label="Search pages, sections or actions" data-command-search>
      <div class="command-palette__list" data-command-list></div>
    </section>
  `;

  document.body.append(trigger, palette);

  const input = palette.querySelector("[data-command-search]");
  const list = palette.querySelector("[data-command-list]");
  const closeItems = palette.querySelectorAll("[data-command-close]");

  function renderCommands(query = "") {
    if (!list) return;
    const normalized = query.trim().toLowerCase();
    const visible = commands.filter((command) => `${command.label} ${command.meta}`.toLowerCase().includes(normalized));

    list.innerHTML = "";
    visible.forEach((command, index) => {
      const item = document.createElement(command.href ? "a" : "button");
      item.className = "command-item";
      if (command.href) {
        item.setAttribute("href", command.href);
      } else {
        item.type = "button";
      ...
      item.innerHTML = `<span>${command.label}</span><small>${command.meta}</small><em>${String(index + 1).padStart(2, "0")}</em>`;
      list.appendChild(item);
    });
  }

  function openPalette() {
    renderCommands(input ? input.value : "");
  ...
  closeItems.forEach((item) => item.addEventListener("click", closePalette));
